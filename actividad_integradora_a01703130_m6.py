...
st.title(':rotating_light: Police Incident Reports from 2018 to 2020 in San Francisco:rotating_light:')
...
st.markdown('Crime locations in San Francisco:cop:')
st.map(subset_data)
st.markdown('Crimes ocurred per day of the week:calendar:')
st.bar_chart(subset_data['Day'].value_counts())
st.markdown('Crimes ocurred per date:calendar:')
st.line_chart(subset_data['Date'].value_counts())
st.markdown('Crimes ocurred per hour:alarm_clock:')
st.line_chart(subset_data['Incident Time'].value_counts())
st.markdown('Type of crimes committed:bar_chart:')
st.bar_chart(subset_data['Incident Category'].value_counts())
st.markdown('Crimes ocurred by year:bar_chart:')
...
st.markdown('Resolution status:male-judge:')
